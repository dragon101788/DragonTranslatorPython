...
Mirrors the Rust tts.rs: runs piper.exe as a subprocess, feeds text via stdin,
...
def _find_voice(lang: str, preferred_voice: Optional[str] = None) -> tuple[str, int]:
    ...
    print(f"[TTS] find_voice: lang={lang} preferred={preferred_voice} dir={vdir}")

    # If user picked a specific voice, use it directly
    if preferred_voice:
        for v in all_voices:
            if v["name"] == preferred_voice:
                model_path = os.path.join(vdir, f"{v['name']}.onnx")
                print(f"[TTS] find_voice: preferred -> {model_path} @ {v['sample_rate']}Hz")
                return (model_path, v["sample_rate"])
    ...
    if not lang or lang == "auto":
        ...
        model_path = os.path.join(vdir, f"{target['name']}.onnx")
...
def list_voices() -> list[dict[str, Any]]:
    """Scan piper-voices/ directory and return installed voice metadata."""
    vdir = voices_dir()
...
def open_voices_dir() -> None:
    """Open the voices directory in File Explorer."""
    vdir = voices_dir()
