...
Mirrors the Rust user_files.rs: reads default-config.json from the runtime
...
from dragon_translator import paths, logger
...
def seed_config(app_root: Path) -> None:
    ...
    (mirrors the original Tauri plugin-store format).
    """
    try:
