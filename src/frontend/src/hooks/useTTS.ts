...
// ---------------------------------------------------------------------------
// TTS hook — Piper with Web Speech API fallback
// ---------------------------------------------------------------------------
...
export function useTTS() {
  ...
  // Singleton listener for tts_complete event from backend
  useEffect(() => {
    if (ttsEventListening) return;
  ...
  const speak = useCallback(
    async (text: string, lang?: string) => {
      ...
      // ---- Piper via backend (non-blocking, returns immediately) ----
      try {
